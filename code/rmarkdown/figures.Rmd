...
aus.sub.PLY <- disaggregate(aus.PLY)
aus.sub.PLY <- SpatialPolygonsDataFrame(aus.sub.PLY, data=data.frame(size=gArea(aus.sub.PLY, byid=TRUE)))
aus.sub.PLY <- aus.sub.PLY[aus.sub.PLY$size >= quantile(aus.sub.PLY$size, 0.9, names=FALSE),]

# fortify polygons
sim.pus.FPLY <- fortify(sim.pus)
aus.FPLY <- fortify(aus.sub.PLY)
...
```{r, echo=FALSE, message=FALSE, fig.height=2, fig.width=2, fig.cap="Example of an attribute space. This environmental attribute space has dimensions relating to annual temperature ($^{\\circ}$C) and rainfall (mm). Letters denote the environmental conditions associated with the geographic locations where four hypothetical populations are found. Points represent demand points. In this space, populations closer to each other have similar environmental conditions."}
## generate data
# population data
...
## plot data
ggplot(aes(x=rainfall, y=temperature), data=populations) +
...
```{r, echo=FALSE, messages=FALSE, results='hide', fig.height=1.6, fig.width=column.width, fig.cap='Distributions of three simulated species. Squares denote planning units. The color of each planning unit indicates the probability that species inhabit it.'}
# prepare data
...
```{r, echo=FALSE, fig.height=1.9, fig.width=column.width, fig.cap='Distribution of environmental variation across Queensland, Australia. Squares denote planning units. Panels show the two main gradients of climatic variation.'}
# prepare data
cs1.pca.FPTS <- ldply(
...
p1 <- ggplot() +
	...
	geom_polygon(data=cs1.pus.FPLY, aes(x=long, y=lat, group=group), fill=NA, color='black', size=0.05) +
	theme_classic() +
  ...
  coord_cartesian(xlim=buffered.range(cs1.pus@bbox[1,], 0.025), ylim=buffered.range(cs1.pus@bbox[2,], 0.025), expand=FALSE) +
...
```{r, echo=FALSE, fig.height=3.8, fig.width=column.width, fig.cap='Distribution of the species used in the first case study. Each panel shows a different species. Squares denote planning units. Planning units occupied by a species are shown in bright blue.'}
# prepare data
...
cs1.pus.occ.letters.DF <- data.frame(
	x=unname(cs1.pus@bbox[1,1] + (diff(cs1.pus@bbox[1,])*0.08)),
	y=unname(cs1.pus@bbox[2,1] + (diff(cs1.pus@bbox[2,])*0.95)),
	species=unique(cs1.pus.occ.FPLY$species),
	letter=paste0('(', letters[seq_along(unique(cs1.pus.occ.FPLY$species))], ')')
)
# make plots
ggplot() +
	geom_polygon(data=aus.FPLY, aes(x=long, y=lat, group=group), fill='grey85', color='grey70', size=0.05) +
...
```{r, echo=FALSE, fig.height=1.9, fig.width=column.width, fig.cap=paste0('Prioritizations for the first case study. Squares represent planning units. Dark green planning units were selected for protection. Panel (a) shows the planning units selected when using ',cs1.params.LST[[MODE]]$amount.target*100,' % amount targets. Panel (b) shows the planning units selected when using ',cs1.params.LST[[MODE]]$amount.target*100,' % amount targets and ',cs1.params.LST[[MODE]]$space.target*100,' % niche targets.')}
# prepare data
...
ggplot() +
	geom_polygon(data=aus.FPLY, aes(x=long, y=lat, group=group), fill='grey85', color='grey70', size=0.05) +
	geom_polygon(data=cs1.pus.sel.FPLY, aes(x=long, y=lat, group=group, fill=Solution), color='black', size=0.05) +
...
```{r, echo=FALSE, fig.height=2.2, fig.width=column.width, fig.cap=paste0('Data used for the second case study. Squares denote planning units. Panel (a) shows all grid cells surveyed by the IntraBioDiv project. Grid cells occupied by ',tolower(cs2.params.LST[[MODE]]$common.name),' are shown in bright blue. The subsequent panels contain only show occupied grid cells. Panel (b) shows the opportunity cost of each planning unit (estimated as the total human population density). Panels (c--d) show the spatial distribution of the ordinated genetic data. These values describe the typical genetic characteristics of individuals in each planning unit. Planning units with similar values/colors contain individuals with similar loci polymorphisms.')}
...
cs2.spp.grid.letters.DF <- data.frame(
	x=unname(cs2.grid.PLY@bbox[1,1] + (diff(cs2.grid.PLY@bbox[1,])*0.07)),
	y=unname(cs2.grid.PLY@bbox[2,1] + (diff(cs2.grid.PLY@bbox[2,])*0.875)),
	letter=('(a)')
)
...
cs2.spp.grid.sub.letters.DF <- data.frame(
	...
	y=unname(cs2.grid.sub.PLY@bbox[2,1] + (diff(cs2.grid.sub.PLY@bbox[2,])*0.89)),
	letter=c('(b)','(c)','(d)')
)
...
p2 <- ggplot() +
	...
	theme(
		...
		legend.position=c(0.25, 0.58), legend.key.height=unit(2.5, 'mm'), legend.key.width=unit(3.25, 'mm'), legend.text=element_text(size=6.0),
...
cs2.sel.grid.sub.letters.DF <- data.frame(
	x=unname(cs2.grid.sub.PLY@bbox[1,1] + (diff(cs2.grid.sub.PLY@bbox[1,])*0.06)),
	y=unname(cs2.grid.sub.PLY@bbox[2,1] + (diff(cs2.grid.sub.PLY@bbox[2,])*0.9)),
	letter=c('(a)','(b)')
)
