...
The control variables for the reliable formulation are the $B$ (1a), $T_{s}$ (1b), $\tau_{sa}$ (1c), $R$, and $M$ variables.
...
The reliable formulation is non-linear. However, the non-linear components can be linearised. First--as discussed in the main text--the expression $X_j X_k$ in (3a) can be linearised using methods described by Beyer et al. [-@r426]. Second, the expression $P_{fsijr} Y_{jsijr}$ in (7a) can be linearised using techniques described by Sherali and Alameddine [-@r20] as implemented in Cui et al. [-@r16].
