...
Prioritisations must capture the ecological and evolutionary processes that sustain biodiversity [@r3; @r255; @r9]. Ecological processes are important for biodiversity to persist over short time-scales. For instance, predator-prey processes can mediate ecosystem structure and function through top-down control [@r476]. Additionally, migratory processes allow many species to access suitable habitat throughout the year [r477]. On the other hand, evolutionary processes are important for biodiversity to persist over long time-scales. Adaptive evolutionary processes foster resilience against environmental change [@r3; @r43], and neutral evolutionary processes maintain genetic diversity [@r5; @r67; @r63]. In recent decades, a wealth of data relevant to biodiversity processes has become available [eg. bioclimatic, distribution, genetic, phylogenetic, and trait data; @r406; @r33; @r460; @r453]. This is sparked significant interest in using this data to make better conservation decisions [@r473; @r9; @r448; @r2; @r68; @r470; @r469; @r247; @r488; @r468; @r407; @r61; @r475].

<!-- current methods for preserving biodiversity processes -->
Today, the most widely used conservation planning tools focus on biodiversity patterns [\texttt{Marxan} and \texttt{Zonation}; @r22; @r429]. The cornerstone of these tools is that decision makers can set targets for a proportion of the geographic range of species, or ecosystems (termed features). To incorporate some information on biodiversity processes, current methods involve partitioning the features into different groups as a pre-processing step [@r9; @r448; @r247]. These groups are partitioned based on a proxy variable that relates to a biodiversity process of interest [@r473]. For instance, species ranges' have been partitioned according to habitat discontinuities to capture adaptive processes [@r2]. However, by partitioning features into different groups, it is not clear how well a given solution samples the original proxy variable, and ultimately, how well the solution captures the biodiversity process of interest. To overcome this limitation, Faith and Walker [-@r218] developed a decision support tool (\texttt{DIVERSITY}) to explicitly sample proxy variables to secure biodiversity processes. However, this tool is not widely used today, in part, because it cannot accommodate explicit targets on biodiversity patterns for multiple features.

<!-- aims of this paper -->
One of the key issues in reserve selection is the lack of unified a decision support tool that uses explicit and separate targets for biodiversity patterns and processes. To begin to fill this gap, we united the ideas underpinning \texttt{DIVERSITY} and \texttt{MARXAN} into new optimisation problems, and implemented them in the \texttt{raptr R} package. This \texttt{R} package provides decision makers with the tools to generate prioritisations that preserve both biodiversity patterns and processes. Here, we aim to the provide an in-depth understanding of its inner workings and explore its functionality.
