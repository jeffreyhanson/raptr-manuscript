...
1. An underlying aim in conservation planning is to maximise the long-term persistence of biodiversity. To fulfill this aim, the processes that sustain biodiversity must be conserved. Many biodiversity processes can be preserved by conserving a representative sample of each biodiversity feature (species or ecosystem). This idea has for decades been encapsulated in the principle of \"representativeness\". For example, representing the range of genetic diversity or ability to persist in a range of climatic conditions might be crucial for the long-term persistence of a species in a world where environmental change is accelerating. Yet current approaches overwhelmingly focus on \"adequacy\", the concept of ensuring a target proportion of the distribution of a biodiversity feature is reserved.

2. To address this issue, we developed the \\texttt{raptr: Representative and Adequate Prioritisation Toolkit in R} to guide reserve selection using conventional area protection targets as well as explicit targets for representing intra-specific biodiversity processes. Users set \"amount targets\" to preserve an adequate proportion of habitat for each species. They also set \"representation targets\" to secure a representative sample of variation within features reflect a biodiversity process (eg. genetic characteristics between individuals in areas, or variation in climatic conditions between inhabited areas). We explored the functionality of this package using a simulated study and two case studies. In each study, we generated solutions that secured a proportion of the species' distributions using amount targets--representing conventional reserve selection--and compared them with solutions generated using both amount and space targets.

3. We show that explicitly considering biodiversity processes in reserve selection can substantially change the solution. The simulations suggest that including representativeness in conservation planning is important where biodiversity features have multi-modal distributions in an attribute space. The case studies show that explicitly considering representation-based targets for biodiversity processes can return solutions that could be far more effective in achieving long term biodiversity persistence and do not require much more area to be reserved than traditional approaches. 
...
Perhaps the most fundamental aim of conservation is to maximise the long-term persistence of biodiversity [@r423; @r255]. To achieve this, conservation actions must preserve biodiversity patterns (eg. populations, species, ecosystems), but also crucially the processes that sustain them. One of the major tangible achievements of modern conservation has been the act of setting aside areas for preservation [@r440]. Reserve networks buffer species from gross threatening processes and set the stage for enhanced management interventions [@r465]. Since the resources available for conservation action are limited, protected area networks must be sited in places that satisfy conservation objectives for minimal cost [@r255]. However, despite the importance of biodiversity processes, current approaches for incorporating them into reserve selection are limited.

<!-- biodiversity processes are important -->
Prioritisations must capture the ecological and evolutionary processes that sustain biodiversity [@r3; @r255; @r474]. Ecological processes are important for biodiversity to persist over short time-scales. For instance, predator-prey processes can mediate ecosystem structure and function through top-down control [@rXXX]. Additionally, migratory processes  allow many species to access suitable habitat for breeding [@rXXX]. On the other hand, evolutionary processes are important for biodiversity to persist over long time-scales. Adaptive evolutionary processes foster resilience against environmental change [@r3; @r43], and neutral evolutionary processes maintain genetic diversity [@r5; @r67; @r63]. In recent decades, a wealth of data relevant to biodiversity processes has become available to conservation planners [eg. bioclimatic, distribution, genetic, phylogenetic, and trait data; @rXXX; @r33; @r460; @r453]. This boom has resulted in significant progress towards incorporating both ecological and evolutionary processes into conservation planning [@r473; @r474; @r448; @r2; @r68; @r470; @r469; @r247; @r488; @r468; @r407].

<!-- current methods for preserving biodiversity processes -->
Many decision support tools have been developed to help identify cost-effective solutions [eg. \texttt{ConsNet}, @r446; \texttt{Marxan}, @r22; \texttt{Zonation}, @r429]. Typically, these tools are used to deliver a prioritisation that secures a set of species of interest to the conservation planner (termed target species or features). Given a set of species, decision makers use these tools to generate a prioritisation that secures an adequate proportion of each species' range. To accommodate biodiversity processes, current methods involve partitioning species' ranges into different groups as a pre-processing step to render a new set of "pseudo-species" [eg. at habitat discontinuities; @r2]. However, this approach suffers two main limitations. First, splitting species' ranges into discrete groups is not a trivial task. Data on biodiversity processes are often continuous, hierarchical, and hyper-dimensional, and so may not be able to be reduced to a few groups without significant information loss [@r218]. Second, by splitting the proxy variable into groups, the decision maker cannot directly assess how well a prioritisation actually samples variation in the original proxy variable. Simply setting targets for representing each of the pseudo-species does not necessarily guarantee that the original proxy variable been well sampled--and by extension that the biodiversity process has been preserved.

<!-- aims of this paper -->
One of the key issues in reserve selection is the lack of a unifying decision support tool that can accommodate data on biodiversity patterns and processes. To begin to fill this gap, we present the \texttt{raptr R} package. This \texttt{R} package uses novel formulations of the reserve selection problem to provide decision makers with the tools to generate prioritisations that preserve both biodiversity patterns and processes. Here, we aim to define the concepts that underpin this \texttt{R} package and explore its functionality.
...
# Data accessibility
The \texttt{raptr R} package can be downloaded from The Comphrehensive R Archive Network (\url{https://cran.r-project.org/}). All data, code, and results are stored in an online repository (\url{https://www.github.com/paleo13/raptr-manuscript}) to permit replication and validation of this study.
