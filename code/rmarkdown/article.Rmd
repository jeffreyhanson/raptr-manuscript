...
1. An underlying aim in conservation planning is to maximise the long-term persistence of biodiversity. To fulfill this aim, the processes that sustain biodiversity must be conserved. Many biodiversity processes can be preserved by conserving a representative sample of each biodiversity feature (species or ecosystem). For example, representing ability for individuals to persist in a range of climatic conditions might be crucial for the long-term persistence of a species in a world where environmental change is accelerating. Yet current approaches overwhelmingly focus on securing a target proportion of features' geographic ranges; little attention is directed towards targeting a representative sample of them.

2. To address this issue, we developed the \\texttt{raptr: Representative and Adequate Prioritisation Toolkit in R} to guide reserve selection using explicit targets for representing biodiversity processes as well as area protection targets. Users set \"space targets\" to secure a representative sample of each feature within an attribute space that reflects a biodiversity process (eg. an attribute space expressing variation in genetic characteristics between individuals, or variation in climatic conditions between areas). Users also set \"amount targets\"--similar to conventional reserve selection methods--to secure a target proportion of the feature's geographic distribution. We demonstrated the functionality of this package using a simulated study and two case studies. In each study, we generated solutions that secured a proportion of the species' distributions using amount targets--representing conventional reserve selection--and compared them to solutions generated using both amount and space targets.

3. We show that explicitly considering biodiversity processes in reserve selection can substantially change the solution. The simulations suggest that including representativeness in conservation planning is important where biodiversity features have multimodal distributions in an attribute space. The case studies show that explicitly considering space-based targets for biodiversity processes can result in solutions that could be far more effective in achieving long term biodiversity persistence and do not require a much greater area reserved than traditional approaches. 
...
Prioritisations must capture the ecological and evolutionary processes that sustain biodiversity [@r3; @r255; @r9]. Ecological processes are important for biodiversity to persist over short time-scales. For instance, predator-prey processes can mediate ecosystem structure and function through top-down control [@r476]. Additionally, migratory processes allow many species to access suitable habitat throughout the year [@r477]. On the other hand, evolutionary processes are important for biodiversity to persist over long time-scales. Adaptive evolutionary processes foster resilience against environmental change [@r3; @r43], and neutral evolutionary processes maintain genetic diversity [@r5; @r67; @r63]. In recent decades, a wealth of data relevant to biodiversity processes has become available [eg. bioclimatic, distribution, genetic, phylogenetic, and trait data; @r33; @r460; @r478; @r453]. This has sparked significant interest in using this data to make better conservation decisions [@r68; @r9; @r473; @r247; @r448; @r468; @r2; @r407; @r61; @r470; @r475; @r469].

<!-- current methods for preserving biodiversity processes -->
Today, the most widely used conservation planning tools focus on biodiversity patterns [\texttt{Marxan} and \texttt{Zonation}; @r429; @r22]. The cornerstone of these tools is that they allow decision makers to set targets to obtain solutions that secure a proportion of the geographic range of biological features (populations, species, or ecosystems of conservation interest). To incorporate some information on biodiversity processes, current methods involve partitioning the features into different groups as a pre-processing step [@r9; @r247; @r448]. These groups are partitioned based on a proxy variable that relates to a biodiversity process of interest [@r473]. For instance, species ranges' have been partitioned according to habitat discontinuities to capture adaptive processes [@r2]. However, by partitioning features into different groups, it is not clear how well a given solution samples the original proxy variable, and ultimately, how well the solution captures the biodiversity process of interest. This limitation is also present in some of the earliest reserve selection methods [eg. @r295], and to overcome it, Faith and Walker [-@r218] developed a decision support tool (\texttt{DIVERSITY}) to explicitly sample proxy variables to secure biodiversity processes. However, this tool is not widely used today, in part, because it cannot accommodate explicit targets on biodiversity patterns for multiple features (unlike and \texttt{Marxan} and \texttt{Zonation}).
